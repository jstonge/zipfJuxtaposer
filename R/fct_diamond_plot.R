...
diamond_plot <- function(mixedelements, alpha, system_names) {
  ...
  anno_names_right <- geom_text(data = sel_names[sel_names$elem=="right",],
                                aes(.data$x1, .data$y1, label = .data$types, alpha = .data$deltas/sum(.data$deltas)),
                                nudge_x = -2.8, nudge_y = 3, angle = (-1 * rotation),
                                show.legend = FALSE)
  
  anno_names_left <- geom_text(data = sel_names[sel_names$elem=="left",],
  ...
  mylabels <- function(x){ sprintf("%.0f", new_labs[new_labs$old_labs %in% x,]$new_labs) }
